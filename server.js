...
app.get(`/api/weather`, async (req, res) =>{
    try{
        ...
        // Weather alerts for the specific point are more precise than state-wide alerts.
        const weatherAlertURL = new URL(`https://api.weather.gov/alerts/active`);
        weatherAlertURL.searchParams.set("point", `${latData},${longData}`);
        let weatherAlertData = { features: [] };
        let alertStatus = "unavailable";
        let alertError = null;
        ...
        try {
          ...
          if (weatherAlertResponse.ok) {
            ...
            alertStatus = "error";
            alertError = `Weather alerts request failed with status ${weatherAlertResponse.status}`;
            console.error(alertError);
          }
        } catch (alertErr) {
          alertStatus = "error";
          alertError = "Weather alerts request failed before a response was returned.";
